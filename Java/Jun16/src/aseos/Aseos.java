...
public class Aseos {
	...
	private Semaphore mutex= new Semaphore (1, true), clientePuedeEntrar = new Semaphore (0, true),
	...
	public void entroAseo(int id) throws InterruptedException {
		mutex.acquire();
		while(equipoDentro || equipoEsperando) {
			cEspera++;
			System.out.println("Cliente "+id+ " en espera.");
			mutex.release();
			clientePuedeEntrar.acquire();
			mutex.acquire();
		}
